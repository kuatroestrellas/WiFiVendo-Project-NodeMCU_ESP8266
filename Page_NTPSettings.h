...
<tr><td align="right">NTP Server:</td><td><input type="text" id="ntpserver" name="ntpserver" maxlength="172" value="80"></td></tr>
<tr><td align="right">Update:</td><td><input type="text" id="update" name="update" size="3" maxlength="6" value="10"> minutes (0=disable)</td></tr>
...
void send_NTP_configuration_html()
{
	...
	if (server.args() > 0 )  // Save Settings
	{
		...
		for ( uint8_t i = 0; i < server.args(); i++ ) {
			...
			if (server.argName(i) == "dst") config.daylight = false; 
		}
		WriteConfig();
		firstStart = true;
	}
