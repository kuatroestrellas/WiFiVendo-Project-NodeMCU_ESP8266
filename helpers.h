...
int EEPROMReadInt(int address, int value)
{
...
const char *myHostname = "PisoWifiVendo";  
  if (!isIp(server.hostHeader()) && server.hostHeader() != (String(myHostname) + ".local")) {
    Serial.println(F("Request redirected to captive portal"));
    server.sendHeader("Location", String("http://192.168.4.1") , true);
//    server.sendHeader("Location","/"), true); // Add a header to respond with a new location for the browser to go again
